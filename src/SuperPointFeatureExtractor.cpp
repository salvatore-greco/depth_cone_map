...
SuperPointFeatureExtractor::SuperPointFeatureExtractor(const std::string& config_path, const std::string& weight_path, const rclcpp::Logger& logger, std::shared_ptr<MessageContainer> message_container)
...
bool SuperPointFeatureExtractor::isInsideBoundingBox(const double x_feature, const double y_feature, const std::pair<cv::Point, cv::Point>& bb) const {
    return (x_feature >= (bb.first.x / scale_factor_x)) && (x_feature <= (bb.second.x / scale_factor_x)) &&
           (y_feature >= (bb.first.y / scale_factor_y)) && (y_feature <= (bb.second.y / scale_factor_y));
}
